...
class ImportThread(QThread):
    ...
    def _subfolders_belong_to_single_subject(self, subfolders):
        """
        Heuristics: Subfolders belong to the same subject if:
        - there is at most 1 non-empty PatientID AND at most 1 non-empty PatientName, or
        - Name + BirthDate + Sex match (<=1), or
        - a typical MR+PT pattern ({'MR','PT'} mode) with consistent BirthDate/Sex and
        no clear signs of multiple subjects.
        """
        import re
