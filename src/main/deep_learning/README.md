...
4. **Obtain Model Checkpoints**
   ...
   To function correctly, the Deep Learning module requires specific pre-trained model files (checkpoints, e.g., last.ckpt or epoch=xx.ckpt).

   Due to GitHub storage policies and file size limitations, these files are not included in the repository.

   > **Action**: Please contact the software developers privately to request the necessary checkpoint files.

   > **Placement**: Once received, place the files in the `deep_learning/checkpoints/fold3/` folder (or the specific directory indicated by the developers).
